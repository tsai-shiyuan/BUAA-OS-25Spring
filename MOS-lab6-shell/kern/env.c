...
void extend_share_vars() {
	...
	for (int j = 0; j < varnum; j++) {
		printk("name: %s, value: %s\n", envvars[j].name, envvars[j].value);
	}
	*/
}
...
static void map_segment(Pde *pgdir, u_int asid, u_long pa, u_long va, u_int size, u_int perm) {
	...
	for (int i = 0; i < size; i += PAGE_SIZE) {
		...
		page_insert(pgdir, asid, pa2page(pa + i), va + i, perm);		// 为什么
	}
}
...
static int load_icode_mapper(void *data, u_long va, size_t offset, u_int perm, const void *src,
			     size_t len) {
	struct Env *env = (struct Env *)data;	// 还原为Env, 本来在load_icode中就传入的Env
	struct Page *p;
	int r;
...
struct Env *env_create(const void *binary, size_t size, int priority) {
	...
	load_icode(e, binary, size);	// 为进程加载 ELF 程序
